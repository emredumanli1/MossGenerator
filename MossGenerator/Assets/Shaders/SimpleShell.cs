...
public class SimpleShell : MonoBehaviour {
    ...
    void OnEnable() {
        ...
        if (noiseTexture != null) {
            shellMaterial.SetTexture("_NoiseTex", noiseTexture);
        }
        else {
            Debug.LogWarning("Noise texture not assigned!");
        }

        if (mossTexture != null) {
            shellMaterial.SetTexture("_MossTex", mossTexture);
        }

        else {
            Debug.LogWarning("Moss texture not assigned!");
        }

        if (rotationTexture != null)
        {
            shellMaterial.SetTexture("_RotationTex", rotationTexture);
        }

        else
        {
            Debug.LogWarning("Rotation texture not assigned!");
        }
        ...
        for (int i = 0; i < shellCount; ++i) {
            ...
            shells[i].GetComponent<MeshRenderer>().material.SetFloat("_ShellOffset", shellOffset);
            ...
            shells[i].GetComponent<MeshRenderer>().material.SetFloat("_GrowthSpeed", growthSpeed);
        }
    }
    ...
    void Update() {
        ...
        if (growth < 1.0f) {
            growth += Time.deltaTime * growthSpeed * 0.1f;
        }
        else if (loopGrowth)
        {
            growth = 0.2f;
        ...
        if (updateStatics) {
            for (int i = 0; i < shellCount; ++i) {
                ...
                shells[i].GetComponent<MeshRenderer>().material.SetVector("_ShellColor", shellColor);
                shells[i].GetComponent<MeshRenderer>().material.SetFloat("_GrowthSpeed", growthSpeed);
                shells[i].GetComponent<MeshRenderer>().material.SetFloat("_Growth", growth);
            }
        }
    }
